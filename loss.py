...
class CE:
    ...
    def computefreq(self, batch):
        seqs, labels = batch
        #print(labels)
        scores = self.model(seqs)  # B * N lastrep, attn_encoded,
        labels = labels.view(-1).long()
        loss = self.ce(scores, labels)
        return loss

    def computeimg(self, batch):
        eeg, labels, img_emb, neg_img_emb = batch
        # print(labels)
        lastrep, eeg_img_encode, scores = self.model(eeg)  # B * N
        labels = labels.view(-1).long()
        loss = self.ce(scores, labels)
        return loss
