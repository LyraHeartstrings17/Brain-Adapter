...
class PatchEmbed1D(nn.Module):
    ...
    def __init__(self, time_len=512, patch_size=4, in_chans=128, embed_dim=1024):
        super().__init__()
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.proj(x)  # put embed_dim at the last dimension
...
class MAEforEEG(nn.Module):
    ...
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
        ...
        self.num_patches = num_patches
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))  # 1024
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim),
                                      requires_grad=False)  # fixed sin-cos embedding 129 1024
        ...
        if use_nature_img_loss:
            ...
            self.nature_img_decoder_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, decoder_embed_dim),
                                                             requires_grad=False)  # fixed sin-cos embedding
    ...
    def initialize_weights(self):
        ...
        if self.use_nature_img_loss:
            nature_img_decoder_pos_embed = ut.get_1d_sincos_pos_embed(self.nature_img_decoder_pos_embed.shape[-1],
