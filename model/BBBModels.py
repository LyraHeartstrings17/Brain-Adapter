...
class OnlyTimeEncoder(nn.Module):
    def __init__(self, pretrained_model_time):
        super(OnlyTimeEncoder, self).__init__()
        ...
        self.fc01 = nn.Linear(1024, 40)

    def forward(self, x):
        time_feature = self.pretrained_model_time(x)
        x = self.fc01(time_feature)
        scores = x
        return scores


class OnlyFreqEncoder(nn.Module):
    def __init__(self, pretrained_model_freq):
        super(OnlyFreqEncoder, self).__init__()

        self.pretrained_model_freq = pretrained_model_freq

        self.fc01 = nn.Linear(256, 40)

    def forward(self, x):
        lstmcls, freq_feature = self.pretrained_model_freq(x)
        x = self.fc01(freq_feature)
        scores = x
        return scores
...
class ClsTimeFreqEncoder(nn.Module):
    def __init__(self, pretrained_model_time, pretrained_model_freq, img_loss=False):
        super(ClsTimeFreqEncoder, self).__init__()

        self.pretrained_model_time = pretrained_model_time
        self.pretrained_model_time.nocliptune = True
        self.pretrained_model_time.linear_proba = False
        self.pretrained_model_freq = pretrained_model_freq
        self.fc01 = nn.Linear(1024 + 256, 768)
        self.fc02 = nn.Linear(768, 40)
        self.fc03 = nn.Linear(1024 + 256, 40)
...
class FreqEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        e_output, e_hidden_state = self.lstm(x)
        x = e_output[0][:, -1, :]
        reps = x
        # Forward output
...
if __name__ == '__main__':
    # Example usage:
    input_dim = 220  # 特征维度
    hidden_dim = [128, 96, 64]  # 隐藏层维度
    kernel_size = [3, 3, 3]  # 卷积核大小
    num_layers = 3  # LSTM层数
    # 创建一个随机输入张量，形状为 (batch_size, time_steps, features_dim)
    input_tensor = torch.randn(1, 128, 220)  # 例如：5个样本，15个时间步，每个时间步10个特征
    # 创建模型
    encoder = ConvLSTM(input_dim, hidden_dim, kernel_size, num_layers)

    # 前向传播
    e_output, hidden_state = encoder(input_tensor)
    print("E_Output shape:", e_output[0].shape)
    d_input_dim = 64  # 特征维度
    d_hidden_dim = [96, 128, 220]  # 隐藏层维度
    d_kernel_size = [3, 3, 3]  # 卷积核大小
    decoder = ConvLSTM(d_input_dim, d_hidden_dim, d_kernel_size, num_layers)
    d_output, d_hidden_state = decoder(e_output[0])
    print("D_Output shape:", d_output[0].shape)
    print("loss:", F.mse_loss(d_output[0], input_tensor))
